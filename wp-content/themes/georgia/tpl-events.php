    ...
    <div class="row">
		...
		<?php
	        ...
	        //foreach ($queryEvents as $event) {
				...
				array_push($added,$month." ".$year);
				array_push($datenum,substr($datetime, 2, 2).$year);
	    ?>
        <!-- Month / Year Headers -->
       ...
       <?php
		...
		$dateAdded = arrayUnique($added, false);
		$datenumAdded = arrayUnique($datenum);
		$i = 0;
		foreach ($datenumAdded as $numadded) {
		?>
			<span class='tribe-events-list-separator-month'><span><?php echo $dateAdded[$i];?></span></span>
		<?php
		$i++;
        $argevent = array(
			  'post_type'      => 'post',
